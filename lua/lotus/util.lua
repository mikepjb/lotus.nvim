local hsluv = require("lotus.hsluv")
...
-- local ns = vim.api.nvim_create_namespace("lotus")
...
function util.debug(colors)
  colors = colors or require("lotus.colors")
  -- Dump unused colors
...
function util.onColorScheme()
  if vim.g.colors_name ~= "lotus" then
    vim.cmd([[autocmd! lotus]])
    vim.cmd([[augroup! lotus]])
  end
end
...
function util.autocmds(config)
  vim.cmd([[augroup lotus]])
  vim.cmd([[  autocmd!]])
  vim.cmd([[  autocmd ColorScheme * lua require("lotus.util").onColorScheme()]])
  if config.dev then
    vim.cmd([[  autocmd BufWritePost */lua/lotus/** nested colorscheme lotus]])
  end
